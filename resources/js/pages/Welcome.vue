  ...
  <div class="min-h-screen bg-[#0a0a0a] text-white antialiased">
      ...
      <div class="mx-auto flex max-w-[1200px] items-center justify-between gap-3 px-4 py-3">
        ...
        <div class="flex items-center gap-2">
          ...
          <template v-else>
            ...
            <Link :href="route('student.login')" class="btn-solid"> Área do Estudante </Link>
            <Link v-if="!$page.props.auth.user" :href="route('login')" class="btn-outline"> Área Administrativa </Link>
            <Link v-else :href="route('students')" class="bbtn-solid"> Área Administrativa </Link>
          </template>
        </div>
      </div>
    </header>
    ...
    <section id="home" class="relative mx-auto mt-8 max-w-[1200px] px-4">
      <div class="relative overflow-visible rounded-[28px] border border-white/10">
        ...
        <!-- glows -->
        <div class="pointer-events-none absolute -left-24 -top-20 z-10 h-72 w-72 rounded-full bg-[#635BFF]/30 blur-3xl">
        </div>
        <div
          class="pointer-events-none absolute -right-20 -bottom-16 z-10 h-72 w-72 rounded-full bg-[#FF58BD]/25 blur-3xl">
        </div>

        <!-- conteúdo -->
        <div
          class="relative z-20 flex h-[560px] flex-col items-center justify-center px-6 text-center md:h-[600px] lg:h-[640px] pb-24 md:pb-28 lg:pb-32">
        ...
        <div
          class="pointer-events-none absolute left-1/2 z-30 -translate-x-1/2 bottom-[-88px] md:bottom-[-96px] lg:bottom-[-104px]">
...
.hero-title {
  ...
  /* tracking mais fechado, igual Figma */
}
